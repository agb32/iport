...
cameraParams[4*ncam:5*ncam]=1#byteswapInt (1 for ocam)
cameraParams[5*ncam:6*ncam]=50#priority
cameraParams[6*ncam]=1#affin el size
cameraParams[6*ncam+1:7*ncam+1]=0xfc0fc0#affinity
cameraParams[7*ncam+1]=namelen#number of bytes for the name.
cameraParams[7*ncam+2:7*ncam+2+(namelen+3)//4].view("c")[:]=camNames
cameraParams[7*ncam+2+(namelen+3)//4]=0#record timestamp

rmx=numpy.random.random((nacts,ncents)).astype("f")
ab=numpy.array([1056,121]).astype("i").byteswap()
camCommand="DigitizedImageWidth=%d;DigitizedImageHeight=%d;"%(ab[0],ab[1])
#camCommand=None
